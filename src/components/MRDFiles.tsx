...
export default function MRDFiles({ files, onUpdateFileNotes, onRefresh, onShowToast }: MRDFilesProps) {
  ...
  const filteredFiles = files.filter(file => {
    ...
    const matchesDoc = docFilter === '' || file.authoredBy === docFilter;
    const matchesSecurity = securityFilter === '' || file.securityLevel === securityFilter;

    return matchesSearch && matchesDocType && matchesDept && matchesStatus && matchesDoc && matchesSecurity;
  });
      ...
      <div className="px-6 md:px-8 py-3 bg-white/35 backdrop-blur-sm border-b border-white/40 flex flex-col md:flex-row md:items-center justify-between gap-4 select-none animate-fadeIn">
        ...
        <div className="relative flex-grow max-w-md">
          ...
          <input 
            ...
            className="w-full h-9 pl-10 pr-10 bg-white/70 border border-white rounded-xl text-xs text-slate-800 placeholder-slate-400/80 focus:ring-2 focus:ring-blue-500/10 outline-none transition-all shadow-sm"
          />
          {searchTerm && (
            <button
              type="button"
              onClick={() => setSearchTerm('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-slate-200/60 hover:bg-slate-200 text-slate-500 hover:text-slate-800 transition-colors flex items-center justify-center cursor-pointer active:scale-90"
              title="Clear search"
            >
              <X className="w-3 h-3" />
            </button>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="relative">
            <select 
              ...
              className="h-9 pl-3 pr-8 bg-white/70 border border-white rounded-xl text-xs font-bold text-slate-700 outline-none cursor-pointer hover:bg-white transition-all appearance-none shadow-sm"
            >
              <option value="">Clinician Author</option>
              {Array.from(new Set(files.map(f => f.authoredBy))).map(doc => (
          ...
          <div className="relative">
            <select 
              ...
              className="h-9 pl-3 pr-8 bg-white/70 border border-white rounded-xl text-xs font-bold text-slate-700 outline-none cursor-pointer hover:bg-white transition-all appearance-none shadow-sm"
            >
              <option value="">Security Tier</option>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-50/50 pb-20">
          {filteredFiles.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-16 text-center text-slate-500">
              ...
              <button 
                onClick={() => { setSearchTerm(''); setDocTypeFilter(''); setDeptFilter(''); setStatusFilter(''); setDocFilter(''); setSecurityFilter(''); }}
                ...
                {filteredFiles.map((file) => {
                  ...
                  return (
                    <tr 
                      ...
                      className={`group focus:outline-none cursor-pointer border-l-4 transition-all ${
                        isCurrentSelection 
                          ? 'bg-blue-50/45 border-l-blue-600' 
                          : 'bg-transparent border-l-transparent hover:bg-white/35'
                      }`}
                    >
